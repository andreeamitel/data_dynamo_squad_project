...
data "aws_iam_policy_document" "secretmanager_get_secret_policy" {
  statement {
    ...
    resources = ["${var.processed_bucket_arn}","${aws_secretsmanager_secret.database_creds_secret_load.arn}", "arn:aws:secretsmanager:eu-west-2:767397913254:secret:load_database_creds-hQdF6I"]
  }
}
