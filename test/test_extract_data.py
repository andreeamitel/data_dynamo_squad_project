...
def test_returns_list_of_dictionaries():
    ...
    result2 = [type(returned_data) is dict for returned_data in extract_data(
        "currency", my_mock, datetime.now())]
...
def test_returns_dictionary():
    ...
    assert result == [
        {"currency_id": 1, "currency_code": "GBP", "created_at": "2024-02-13", "last_updated": "2024-02-13"
         }
    ]
...
def test_returns_error():
    ...
    assert extract_data("currency", my_mock, datetime.now(
...
def test_returns_using_timestamp_query():
    ...
    test_database = Connection(
        user, database="test_sales_orders", password=pg_password)
    result = extract_data("currency", test_database, '2024-02-13 00:00:00')
    test_database.close()
    assert result == [{'currency_id': 2, 'currency_code': 'EUR', 'created_at': '2024-02-14T09:00:00', 'last_updated': '2024-02-14T09:00:00'}, {'currency_id': 3, 'currency_code': 'USD',
                                                                                                                                               'created_at': '1999-01-08T04:05:06', 'last_updated': '2024-02-14T09:00:00'}, {'currency_id': 4, 'currency_code': 'AUD', 'created_at': '2024-02-14T09:00:00', 'last_updated': '1999-01-08T04:05:06'}]
