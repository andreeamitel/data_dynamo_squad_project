...
@patch("src.transform.lambda_handler.get_latest_data", return_value = { "currency" :
            {"currency": [
                {
                    "currency_id": 1,
                    "currency_code": "GBP",
...
@mock_aws
def test_dim_currency(mock_python_to_parquet, mock_dim_currency, mock_get_latest_data, create_bucket1, create_object, secretmanager):
   lambda_handler(test_event, test_context)
   mock_dim_currency.assert_called_once_with(mock_get_latest_data.return_value["currency"])

@pytest.mark.describe("lambda_handler")
@pytest.mark.it("test that dim_counterparty gets called with correct values")
@patch("src.transform.lambda_handler.get_latest_data", return_value = { "counterparty" :
           { "counterparty": [
            {
                "counterparty_id": 1,
...
@mock_aws
def test_dim_counterparty(mock_dim_counterparty, mock_python_to_parquet, mock_get_latest_data, create_bucket1, create_object, secretmanager):
   lambda_handler(test_event, test_context)
   mock_dim_counterparty.assert_called_once_with(mock_get_latest_data.return_value["counterparty"], mock_get_latest_data.return_value["address"])


@pytest.mark.describe("lambda_handler")
@pytest.mark.it("test that python gets called with correct variables for fact sales")
@patch("src.transform.lambda_handler.get_latest_data", return_value = { "sales_order":
           {
    "sales_order": [
        {
            "sales_order_id": 1,
...
@patch("src.transform.lambda_handler.python_to_parquet")
@patch("src.transform.lambda_handler.fact_sales_order", return_value = ({"fact_sales": { "sales_order": [{
            "sales_record_id": 1,
            "sales_order_id": 1,
...
@mock_aws
def test_fact_sales_parquet(mock_date, mock_fact_sales, mock_python_to_parquet, mock_get_latest_data, create_bucket1, create_object, secretmanager):
   mock_date.now().isoformat.return_value = "2024-02-22T16:41:59.776283"
   lambda_handler(test_event, test_context)
   fact_sales, dim_dates = mock_fact_sales.return_value
   mock_python_to_parquet.assert_called_with(dim_dates, "processed_bucket123", '2024-02-22T16:41:59.776283')
